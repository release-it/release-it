---
title: Custom version plugin recipe
description: A minimal release-it plugin example that reads and bumps a VERSION file and publishes to a custom registry.
sidebar:
  label: Custom version plugin
  order: 7
---

This example plugin reads a `VERSION` file, bumps it, and publishes to a custom package
registry. It only activates when a `./VERSION` file exists.

For the underlying API, see the [Plugin API reference](/release-it/reference/plugin-api/) and
the [Plugin architecture explanation](/release-it/explanations/plugin-architecture/).
...
Enable it in the project that uses it:
