...
import { coerce, increment, isGreater, isGreaterOrEqual, isPrerelease, isValid } from 'verkit';
...
const getIncrementChoices = context => {
  ...
  const choices = types.map(increment => ({
    name: `${increment} (${increment(context.latestVersion, increment, preReleaseId, preReleaseBase)})`,
    value: increment
  }));
  const otherChoice = {
...
class Version extends Plugin {
  ...
  incrementVersion({ latestVersion, increment, isPreRelease, preReleaseId, preReleaseBase }) {
    ...
    const isValidVersion = isValid(increment);
    ...
    if (isPreRelease && !increment && latestIsPreRelease) {
      return increment(latestVersion, 'prerelease', preReleaseId, preReleaseBase);
    }

    if (this.config.isCI && !increment) {
      if (isPreRelease) {
        return increment(latestVersion, 'prepatch', preReleaseId, preReleaseBase);
      } else {
        return increment(latestVersion, 'patch');
      }
    }
    ...
    if (ALL_RELEASE_TYPES.includes(normalizedType)) {
      return increment(latestVersion, normalizedType, preReleaseId, preReleaseBase);
    }

    const coercedVersion = !isValidVersion && coerce(increment);
    if (coercedVersion) {
