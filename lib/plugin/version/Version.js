...
class Version extends Plugin {
  ...
  getIncrementedVersionSync({ latestVersion }) {
    const { increment } = this.options;
    return this.incrementVersion({ latestVersion, increment });
  }

  async getIncrementedVersion({ latestVersion }) {
    const { isInteractive } = this.global;
    const version = this.getIncrementedVersionSync({ latestVersion });
    return version || (isInteractive ? await this.promptIncrementVersion(latestVersion) : null);
  }

  promptIncrementVersion(latestVersion) {
    return new Promise(resolve => {
      this.step({
        prompt: 'incrementList',
        task: increment =>
          increment
            ? resolve(this.incrementVersion({ latestVersion, increment }))
  ...
  incrementVersion({ latestVersion, increment }) {
    const { isPreRelease, preReleaseId } = this.getContext();
