...
const { hasAccess, preview } = require('../../util');
...
class Git extends GitBase {
  ...
  static isEnabled(options) {
    return hasAccess('.git') && (options.commit || options.tag || options.push);
  }
  ...
  async beforeBump() {
    await super.beforeBump();
    const { changelog } = this.getContext();
    preview(this.log, { title: 'changelog', text: changelog });
  }

  async beforeRelease() {
    if (this.options.commit) {
      const rollback = _.once(this.rollback.bind(this));
  ...
  hasUpstreamBranch() {
    return this.exec('git symbolic-ref HEAD', r)
      .then(refs => this.exec(`git for-each-ref --format="%(upstream:short)" ${refs}`, r).then(Boolean))
      .catch(() => false);
  }

  getBranchName() {
    return this.exec('git rev-parse --abbrev-ref HEAD', r).catch(() => null);
  }

  tagExists(tag) {
    return this.exec(`git show-ref --tags --quiet --verify -- "refs/tags/${tag}"`, r).then(() => true, () => false);
  }

  isWorkingDirClean() {
    return this.exec('git diff-index --quiet HEAD --', r).then(() => true, () => false);
  }
  ...
  status() {
    return this.exec('git status --short --untracked-files=no', r).catch(() => null);
  }
