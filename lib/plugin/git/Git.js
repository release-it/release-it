...
class Git extends GitBase {
  ...
  static async isEnabled(options) {
    return options !== false && (await gitExec(['rev-parse', '--git-dir']));
  }
  ...
  async isWorkingDirClean() {
    const execOpts = { options: { ...options, cache: false } };
    const hasCommits = await gitExec(['rev-parse', '--verify', 'HEAD']);

    if (!hasCommits) {
      const status = await this.exec('git status --porcelain', execOpts);
      return status.trim() === '';
    }

    return this.exec('git diff --quiet HEAD', execOpts).then(
      () => true,
      () => false
    );
  }
  ...
  status() {
    return this.exec('git status --short --untracked-files=no', { options: { ...options, cache: false } }).catch(() => null);
  }
