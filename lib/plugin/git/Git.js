...
class Git extends GitBase {
  ...
  isWorkingDirClean() {
    return this.exec('git diff --quiet HEAD', { options: { ...options, cache: false } }).then(
      () => true,
      () => false
    );
  }
  ...
  status() {
    return this.exec('git status --short --untracked-files=no', { options: { ...options, cache: false } }).catch(
      () => null
    );
  }
