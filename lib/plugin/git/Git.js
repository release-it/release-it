...
class Git extends GitBase {
  ...
  async init() {
    console.info({
      diff: await this.exec('git diff-index --quiet HEAD --').catch(err => `exception: ${err}`),
      dirty: !(await this.isWorkingDirClean().catch(err => `exception: ${err}`)),
      isRequiredBranch: await this.isRequiredBranch(this.options.requireBranch).catch(err => `exception: ${err}`)
    });
  ...
  tag({ name = this.options.tagName, annotation = this.options.tagAnnotation, args = this.options.tagArgs } = {}) {
    ...
    return this.exec(['git', 'tag', '--annotate', '--message', message, ...fixArgs(args), n])
