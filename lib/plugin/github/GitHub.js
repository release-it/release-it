...
class GitHub extends Release {
  ...
  async release() {
    ...
    if (isCI) {
      ...
      return this.step({ enabled: !draft, task: () => this.publishRelease(), label: 'GitHub publish release' });
    } else {
      const release = async () => {
      ...
      return this.step({ task: release, label: 'GitHub release', prompt: 'release' });
    }
  }
  ...
  async draftRelease() {
    ...
    const { version, tagName, releaseNotes, reuseRelease } = this.getContext();
    ...
    return this.retry(async bail => {
      try {
        ...
        if (reuseRelease) {
          ...
          release = response.data.find(release => {
            return release.name === name && release.tag_name === tagName;
          });
          if (release) {
            this.log.verbose(`octokit releases#draftRelease: reusing release (${release.url})`);
          } else {
            this.log.verbose(`octokit releases#draftRelease: could not find release to reuse, creating`);
          }
        }
        if (!release) {
          const options = {
        ...
        const { html_url, upload_url, id } = release;
