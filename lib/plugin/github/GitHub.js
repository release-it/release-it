...
class GitHub extends Release {
  ...
  async init() {
    ...
    if (isUpdate) {
      try {
        const { id, html_url, upload_url } = await this.getLatestRelease();
        this.setContext({ isUpdate: true, isReleased: true, releaseId: id, releaseUrl: html_url, upload_url });
      } catch (error) {
  ...
  async getLatestRelease() {
    ...
    try {
      ...
      this.debug(options);
      const response = await this.client.repos.getReleaseByTag(options);
