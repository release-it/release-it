...
module.exports.getPluginNames = () => pluginNames;

module.exports.getPlugins = async (config, global, container) => {
  const context = config.getContext();
  const enabledPlugins = await pluginNames.reduce(async (r, plugin) => {
  ...
  const enabledExternalPlugins = await _.reduce(
    context.plugins,
    async (r, pluginConfig, pluginName) => {
      const result = await r;
      ...
      if (await Plugin.isEnabled(options)) {
        const instance = new Plugin({ namespace, options, global, container });
        debug({ namespace, options });
        result.push(instance);
      }
      return result;
    },
