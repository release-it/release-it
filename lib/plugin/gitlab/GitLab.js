...
class GitLab extends Release {
  constructor(...args) {
    ...
    this.assets = [];
    const { secure, proxy } = this.options;
    ...
    const needsCustomAgent = Boolean(secure === false || certificateAuthorityFile || proxy);

    if (needsCustomAgent) {
      const Dispatcher = proxy ? EnvHttpProxyAgent : Agent;
      const connect = {
        rejectUnauthorized: secure,
        ca: certificateAuthorityFile ? fs.readFileSync(certificateAuthorityFile) : undefined
      };
      // The proxy agent builds the tunnel to the proxy from `proxyTls` and the connection to
      // the target from `requestTls`, so the TLS options have to be passed as `requestTls` too.
      this.certificateAuthorityOption.dispatcher = new Dispatcher({
        connect,
        ...(proxy ? { requestTls: connect } : {})
      });
    }
  }
  ...
  async init() {
    ...
    this.setContext({
      id: String(repoId ?? encodeURIComponent(repo.repository)),
      origin,
  ...
  async createRelease() {
    ...
    const { tagName, git: { tagAnnotation } = {} } = this.config.getContext();
    ...
    const ref = await this.getCurrentCommit();
    const options = {
      json: {
        name,
        ref,
