...
class GitRelease extends GitBase {
  ...
  getInitialOptions(options, namespace) {
    const gitOptions = _.pick(options.git, ['tagName', 'pushRepo', 'remoteUrl', 'changelog']);
  ...
  async beforeRelease() {
    ...
    if (releaseNotes !== changelog) {
      preview(this.log, { title: 'release notes', text: releaseNotes });
    }
  }
