...
class Plugin {
  ...
  exec(command, options, _context = {}) {
    const context = Object.assign(_context, this.config.getContext(), { [this.namespace]: this.getContext() });
  ...
  step(options) {
    ...
    return this.global.isInteractive ? this.showPrompt(opts) : this.spinner.show(opts);
  }

  async afterRelease() {
    if (this.isReleased && typeof this.getReleaseUrl === 'function') {
      this.log.log(`🔗 ${this.getReleaseUrl()}`);
    }
  }
}
