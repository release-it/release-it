...
class npm extends Plugin {
  ...
  async init() {
    ...
    await Promise.race([validations, rejectAfter(REGISTRY_TIMEOUT, e(`Timed out after ${REGISTRY_TIMEOUT}ms.`, docs))]);

    const [isRegistryUp, isAuthenticated, latestVersionInRegistry] = await validations;

    if (!isRegistryUp) {
      throw e(`Unable to reach npm registry (timed out after ${REGISTRY_TIMEOUT}ms).`, docs);
    }
  ...
  async publish({ otp = this.options.otp, otpCallback } = {}) {
    const { publishPath = '.' } = this.options;
