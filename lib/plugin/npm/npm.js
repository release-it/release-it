...
class npm extends Plugin {
  ...
  getInitialOptions(options, namespace) {
    ...
    let program;
    switch (true) {
      case options[namespace].yarn === true:
        program = 'yarn';
        break;
      case options[namespace].pnpm === true:
        program = 'pnpm';
        break;
      case hasAccess(LOCKFILE_YARN) && options[namespace].yarn !== false:
        program = 'yarn';
        break;
      case hasAccess(LOCKFILE_PNPM) && options[namespace].pnpm !== false:
        program = 'pnpm';
        break;
      default:
        program = 'npm';
    ...
    options[namespace].program = program;
  ...
  getCommand(command, options) {
    if (command in programs[this.options.program]) {
      return programs[this.options.program][command](options);
    } else {
      return `npm ${command}`;
    }
  }
