...
class npm extends Plugin {
  ...
  async init() {
    ...
    const pmFromPackageManager = typeof packageManager === 'string' ? packageManager.split('@')[0] : null;
    const pmFromDevEngines = devEngines && devEngines.packageManager && devEngines.packageManager.name;
    const checkPackageManager = pmFromDevEngines === 'yarn' || pmFromPackageManager === 'yarn' ? 'yarn' : 'npm';

    this.setContext({ name, latestVersion, private: isPrivate, publishConfig, checkPackageManager });
  ...
  isRegistryUp() {
    ...
    return this.exec(this.buildNpmCommand(args), { options: getOptions() }).then(
      () => true,
      err => {
  ...
  async isCollaborator() {
    ...
    try {
      let npmVersion = await this.exec(this.buildNpmCommand(['--version']), { options: getOptions() });
      ...
      const accessArgs = [...accessCommand, name];
      if (registry) accessArgs.push('--registry', registry);
      const output = await this.exec(this.buildNpmCommand(accessArgs), { options: getOptions() });
  ...
  async getLatestRegistryVersion() {
    ...
    const args = ['show', `${name}@${tag}`, 'version'];
    if (registry) args.push('--registry', registry);
    return this.exec(this.buildNpmCommand(args), { options: getOptions() }).catch(() => null);
  }

  getRegistryPreReleaseTags() {
    const args = ['view', this.getName(), 'dist-tags', '--json'];
    return this.exec(this.buildNpmCommand(args), { options: getOptions() }).then(
      output => {
        try {
  ...
  buildNpmCommand(args) {
    const pm = this.getContext('checkPackageManager') || 'npm';
    return pm === 'yarn' ? ['yarn', 'npm', ...args] : `npm ${args.join(' ')}`;
  }
}
