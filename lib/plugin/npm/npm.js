...
const programs = {
  ...
  yarn: {
    ...
    version: ({ version }) => `yarn version ${version}`,
...
class npm extends Plugin {
  ...
  getInitialOptions(options, namespace) {
    ...
    const { yarn, yarn1, pnpm } = options[namespace];
    options[namespace].program = yarn ? 'yarn' : yarn1 ? 'yarn1' : pnpm ? 'pnpm' : 'npm';
  ...
  release() {
    ...
    const command = `${this.options.program} publish`;
  ...
  isAuthenticated() {
    ...
    return this.exec(command, { options }).then(
      output => {
        if (output) {
          if (program === 'yarn') {
            // Ugh, need to parse custom/unreliable output. Example:
            // ➤ YN0000: webpro\n➤ YN0000: Done in 0s 357ms
            const [line] = output.split('\n');
            const username = line.includes(':') ? line.split(':')[1] : line;
            this.setContext({ username: username.trim() });
          } else {
            this.setContext({ username: output.trim() });
          }
        } else {
          this.setContext({ username: null });
        }
        return true;
      },
      err => {
  ...
  async publish({ otp = this.options.otp, otpCallback } = {}) {
    const { program, publishPath = '.', publishArgs, isDryRun } = this.options;
