    ...
    return this.exec([publishPackageManager, ...publishCommand, ...args], {
      ...
      .then(() => {
        if (stage) {
          ...
          const url = entity ? `https://www.npmjs.com/settings/${entity}/staged-packages` : 'https://www.npmjs.com';
          this.log.log(`📦 Staged, not yet live. Approve at ${url} (or \`${publishPackageManager} stage approve\`).`);
        } else {
