...
import { isGreater, isNotEqual } from 'verkit';
...
class npm extends Plugin {
  ...
  async init() {
    ...
    const validations = Promise.all([
      this.isRegistryUp(),
      this.isAuthenticated().catch(err => {
        authenticationError = err;
        return false;
      }),
      this.getLatestRegistryVersion()
    ]);

    await Promise.race([validations, rejectAfter(timeout, e(`Timed out after ${timeout}ms.`, docs))]);

    let [isRegistryUp, isAuthenticated, latestVersionInRegistry] = await validations;
    ...
    if (!isAuthenticated) {
      ...
      const canLogin =
        ...
        process.stdin.isTTY &&
        /code (ENEEDAUTH|E401)\b/.test(authenticationError);

      if (canLogin) {
        await this.login();
        [authenticated, latestVersionInRegistry] = await Promise.race([
          Promise.all([this.isAuthenticated().catch(() => false), this.getLatestRegistryVersion()]),
          rejectAfter(timeout, e(`Timed out after ${timeout}ms.`, docs))
        ]);
      }

      if (!authenticated) throw e('Not authenticated with npm. Please `npm login` and try again.', docs);
    }
    ...
    } else if (isNotEqual(latestVersion, latestVersionInRegistry)) {
      this.log.warn(
  ...
  isAuthenticated() {
    const registry = this.getRegistry();
    const registryArg = registry ? ` --registry ${registry}` : '';
    return this.exec(`npm whoami${registryArg}`, { options: { ...getOptions(), cache: false } }).then(
      output => {
  ...
  login() {
    const registry = this.getRegistry();
    const args = registry ? ['--registry', registry] : [];
    return this.exec(['npm', 'login', ...args], {
      options: { ...getOptions(), write: true, interactive: true, cache: false }
    });
  }
  ...
  getStagedPackagesUrl() {
    ...
    return username ? `https://www.npmjs.com/settings/${username}/staged-packages` : 'https://www.npmjs.com';
  }
  ...
  async publish({ otp = this.options.otp, otpCallback } = {}) {
    ...
    const args = [publishPath, '--tag', tag, ...otpArgs, dryRunArg, ...registryArgs, ...fixArgs(publishArgs)].filter(
      Boolean
    );
