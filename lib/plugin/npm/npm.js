...
const programs = {
  npm: {
    whoami: ({ registry }) => `npm whoami${registry ? ` --registry ${registry}` : ''}`,
    version: ({ version }) => `npm version ${version} --no-git-tag-version`,
    publish: ({ publishPath, tag, otpArg, dryRunArg, publishArgs }) => {
      const args = [publishPath, `--tag ${tag}`, otpArg, dryRunArg, ...fixArgs(publishArgs)].filter(Boolean);
      return `npm publish ${args.join(' ')}`;
    }
  },
  pnpm: {
    whoami: ({ registry }) => `npm whoami${registry ? ` --registry ${registry}` : ''}`,
    version: ({ version }) => `npm version ${version} --no-git-tag-version`,
    publish: ({ publishPath, tag, publishArgs }) => {
      const args = [...fixArgs(publishArgs), publishPath, `--tag ${tag}`].filter(Boolean);
      return `pnpm publish ${args.join(' ')}`;
    }
  },
  yarn: {
    whoami: () => `yarn npm whoami --publish`,
    version: ({ version }) => `yarn version ${version} --no-git-tag-version`,
    publish: ({ tag, publishArgs }) => {
      const args = [`--tag ${tag}`, ...fixArgs(publishArgs)].filter(Boolean);
      return `yarn npm publish ${args.join(' ')}`;
    }
  },
  yarn1: {
    whoami: ({ registry }) => `npm whoami${registry ? ` --registry ${registry}` : ''}`,
    version: ({ version }) => `yarn version --new-version ${version} --no-git-tag-version`,
    publish: ({ tag, publishArgs }) => {
      const args = [`--tag ${tag}`, ...fixArgs(publishArgs)].filter(Boolean);
      return `yarn publish ${args.join(' ')}`;
    }
  }
};
...
class npm extends Plugin {
  ...
  async determineProgram() {
    const { yarn, pnpm } = this.options;
    let program;
    switch (true) {
      case yarn === true:
        program = 'yarn';
      ...
      case pnpm === true:
        program = 'pnpm';
        break;
      case hasAccess(LOCKFILE_YARN) && yarn !== false:
        program = 'yarn';
        break;
      case hasAccess(LOCKFILE_PNPM) && pnpm !== false:
        program = 'pnpm';
        break;
      default:
        program = 'npm';
    ...
    if (program === 'yarn') {
      const yarnVersion = await this.shell.exec('yarn --version', { write: false });
      program = semver.major(yarnVersion) === 1 ? 'yarn1' : program;
    }
    this.setContext({ program });
  }

  getCommand(command, options) {
    const { program } = this.getContext();
    if (command in programs[program]) {
      return programs[program][command](options);
    } else {
      return `npm ${command}`;
    }
  }
  ...
  async bump(version) {
    ...
    const command = this.getCommand('version', { version });
    const task = () => this.exec(command);
    const label = command.split(' ').slice(0, 2).join(' ');
    return this.spinner.show({ task, label });
  }
  ...
  release() {
    ...
    const command = `${program} publish`;
    return this.step({ task: publish, label: command, prompt: 'publish' });
  }
  ...
  isAuthenticated() {
    const registry = this.getRegistry();
    const command = this.getCommand('whoami', { registry });
    return this.exec(command, { options }).then(
      output => {
  ...
  async publish({ otp = this.options.otp, otpCallback } = {}) {
    const { publishPath = '.', publishArgs, isDryRun } = this.options;
    const { private: isPrivate, program, tag = DEFAULT_TAG } = this.getContext();
    ...
    const command = this.getCommand('publish', { publishPath, tag, otpArg, dryRunArg, publishArgs });
    return this.exec(command, { options: { write: program !== 'npm' } })
      .then(() => {
