...
const format = (template = '', context = {}) => _.template(template)(context);
...
const preview = (log, options) => {
  if (!options) return;
  const { title, text } = options;
  if (text) {
    log.obtrusive(`${chalk.bold(_.upperFirst(title))}:${EOL}${text.replace(new RegExp(EOL + EOL, 'g'), EOL)}`);
  } else {
    log.obtrusive(`Empty ${_.lowerCase(title)}`);
  }
};
