...
export async function importModuleConfig(specifier, dir) {
  const req = createRequire(dir.endsWith('/') ? dir : dir + '/');
  let resolved;
  try {
    resolved = req.resolve(specifier);
  } catch (err) {
    throw new Error(`Could not resolve configuration package "${specifier}" from ${dir}`);
  }
  let mod;
  try {
    mod = await import(resolved);
  } catch (err) {
    throw new Error(`Failed to import configuration from package "${specifier}" (${resolved})`);
  }
  let cfg = mod?.default ?? mod?.config ?? mod;
  if (typeof cfg === 'function') {
    cfg = await cfg();
  }
  if (!isObjectStrict(cfg)) {
    throw new Error(`Configuration package "${specifier}" did not export an object`);
  }
  return cfg;
}

export function isLikelyPath(s) {
  if (typeof s !== 'string') return false;
  if (s.startsWith('.') || s.startsWith('/') || s.includes('/')) return true;
  return /(\.m?js|\.cjs|\.json|\.ya?ml|\.toml)$/i.test(s);
}

export function pathExists(baseDir, p) {
  try {
