...
import { coerce, isPrerelease, isValid, normalizeFull, tryParse } from 'verkit';
...
export const parseVersion = raw => {
  ...
  const version = typeof raw === 'string' && isValid(raw) ? raw : normalizeFull(coerce(raw));
  if (!version) return { version: raw, isPreRelease: false, preReleaseId: null };
  const parsed = tryParse(version);
  const isPreRelease = isPrerelease(parsed);
