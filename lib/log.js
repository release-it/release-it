...
import { isObject, upperFirst } from './util.js';
...
class Logger {
  ...
  verbose(...args) {
    const { isExternal } = isObject(args.at(-1)) ? args.at(-1) : {};
  ...
  exec(...args) {
    const { isDryRun: isExecutedInDryRun, isExternal, isCached } = isObject(args.at(-1)) ? args.at(-1) : {};
