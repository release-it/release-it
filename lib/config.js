...
const getLocalConfig = config => {
  ...
  const dir = config.configDir ?? process.cwd();
  const extend = config.extends;

  let localConfig = {};
  if (file === false) return localConfig;
  const resolved = loadConfig({ file, dir, extend });
  debug('Loaded local config', resolved.config);
  return isObjectStrict(resolved.config) ? resolved.config : localConfig;
};

class Config {
  constructor(config = {}) {
    this.constructorConfig = config;
    this.localConfig = getLocalConfig(config);
