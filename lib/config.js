...
const getNpmPackageOptions = () => {
  const npm = {};
  const localPkgOptions = {};

  try {
    const p = require(LOCAL_PACKAGE_PATH);
    npm.version = p.version;
    npm.name = p.name;
    npm.private = p.private;
    Object.assign(localPkgOptions, p['release-it']);
  } catch (err) {
    pkg.publish = false;
    debugConfig(err);
  }

  return { localPkgOptions, npm };
};
...
class Config {
  constructor(options = {}, args) {
    ...
    const { localPkgOptions, npm } = getNpmPackageOptions();
    this.localPkgOptions = localPkgOptions;
    this.npm = npm;
  ...
  parsePreReleaseArgument(cli) {
    if (cli.preRelease) {
      if (cli.increment !== undefined) { cli.increment = getFullIncrement(cli.increment); }
      if (cli.preRelease !== true) { cli.preReleaseId = cli.preRelease; }
      cli.npm = cli.npm || {};
