...
const fileExists = path => {
  ...
  try {
    hasFile = fs.statSync(path).isFile();
  } catch (err) {
    debugConfig(err);
    hasFile = false;
  }
...
const getLocalOptions = localConfigFile => {
  ...
  const hasFile = fileExists(localOptionsPath);

  if (localConfigFile && !hasFile) {
...
const getNpmPackageOptions = () => {
  if (!fileExists(LOCAL_PACKAGE_PATH)) {
    throw new Error(`package.json must exist: ${LOCAL_PACKAGE_PATH}`);
  }

  const pkg = require(LOCAL_PACKAGE_PATH);
