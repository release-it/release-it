...
import { run, runTemplateCommand } from './shell';
...
export function isSubmodule() {
  return run('git rev-parse --show-superproject-working-tree', { isReadOnly: true }).then(stdout => !!stdout, () => false);
}
...
function runChangelogCommand(command) {
  return runTemplateCommand(command, { isReadOnly: true })
    .then(stdout => {
      if (config.isVerbose) {
