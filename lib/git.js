...
class Git {
  ...
  async init() {
    // Simple async/await assignments would be cleaner, but in parallel is (>2 times) faster.
    const [remoteUrl, isWorkingDirClean, hasUpstream, latestTag, isRootDir] = await Promise.all([
      this.getRemoteUrl(),
    ...
    this.isWorkingDirClean = isWorkingDirClean;
    this.hasUpstream = hasUpstream;
    this.latestTag = latestTag;
    this.isRootDir = isRootDir;
    this.repo = remoteUrl && repoPathParse(remoteUrl);
  }
  ...
  async validate() {
    ...
    // Validation *and* assigning results, right here in the middle... Dirty, but efficient.
    await this.init();

    if (!this.remoteUrl) {
      throw new GitRemoteUrlError();
    }
    if (this.options.requireCleanWorkingDir && !this.isWorkingDirClean) {
