...
const isGitRepo = () =>
  run('git rev-parse --git-dir', { isReadOnly: true }).then(stdout => stdout === '.git', () => false);

const tagExists = tag =>
  run(`git show-ref --tags --quiet --verify -- "refs/tags/${tag}"`, { isReadOnly: true }).then(() => true, () => false);

const getRemoteUrl = () =>
  run('git config --get remote.origin.url', { isReadOnly: true }).catch(() => {
...
const stageDir = (baseDir = '.') => run(`git add ${baseDir} --all`);
...
const commit = (path, message, version) =>
  run(`git commit ${config.isForce ? '--allow-empty ' : ''}--message="${format(message, version)}"`, path).catch(
    err => {
      debugGit(err);
