...
function getLatestTag() {
  return run('!git', 'describe --tags --abbrev=0').then(result => {
    ...
    config.setRuntimeOption('prevVersion', latestTag);
    return latestTag;
  });
}
...
function getChangelog(options) {
  ...
  if(options.changelogCommand) {
    if(options.changelogCommand.match(/\[REV_RANGE\]/)) {
      const prevVersion = config.getRuntimeOption('prevVersion') || options.prevVersion;
      const prevTag = util.format(options.tagName, prevVersion);
      return tagExists(prevTag).then(hasTag => {
        const command = options.changelogCommand.replace(/\[REV_RANGE\]/, hasTag ? `${prevTag}...HEAD` : '');
