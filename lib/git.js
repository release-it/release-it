...
export function clone(repo, dir) {
  ...
  return run(`git clone ${cleanRepo} -b ${branch} --single-branch ${dir}`, { isReadOnly: true }).catch(err => {
    ...
    if (changelogCommand.match(/\[REV_RANGE\]/)) {
      ...
      return tagExists(latestTag)
        ...
        .catch(err => {
          ...
          debugGit(err);
          throw new Error(`Could not create changelog from latest tag (${latestTag}) to HEAD.`);
        });
    } else {
