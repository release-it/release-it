...
const getGithubClient = ({ host, token, username, password }) => {
...
const release = ({ version, tagName, repo, changelog = '', github }) => {
  ...
  const { preRelease: prerelease, draft, token, username, password } = github;
  ...
  const githubClient = getGithubClient({ host, token, username, password });
...
const uploadAsset = ({ releaseId, repo, github, filePath }) => {
  ...
  const { owner, project } = repo;
  const { token, username, password } = github;
  const host = github.host || repo.host;    
  const githubClient = getGithubClient({ host, token, username, password });
