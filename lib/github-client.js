...
const getGithubClient = ({ host, proxy }) => {
  if (!githubClients[host]) {
    ...
    const options = {
      version: '3.0.0',
      baseUrl,
      timeout: config.getOption('github.timeout'),
      headers: {
...
const release = ({ version, tagName, repo, changelog = '', github }) => {
  ...
  const { preRelease: prerelease, draft, proxy } = github;
  ...
  const githubClient = getGithubClient({ host, proxy });
...
const uploadAsset = ({ release, repo, github, filePath }) => {
  const host = github.host || repo.host;
  const githubClient = getGithubClient({ host });
  ...
  return retry(
    async bail => {
      try {
        const response = await githubClient.repos.uploadReleaseAsset({
          url,
