 ...
 * - `globcp([['** /*.js'], ['** /bower.json']], {cwd: ['dist/', './']}, targetDir);`
 *
...
function globcp(patterns, options, target) {
    ...
    patternGroups = typeof patterns[0] === 'string' ? [patterns] : patterns;
    patternBases = typeof options.cwd === 'string' ? [options.cwd] : options.cwd;
    ...
    return when.map(patternGroups, function(patternGroup) {
        return patternGroup.map(function(pattern) {
            var index = patternGroups.indexOf(patternGroup),
                patternBase = patternBases[index];
