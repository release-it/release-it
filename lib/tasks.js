...
module.exports = async options => {
  ...
  try {
    ...
    const { latestVersion, version } = await parseVersion(options);

    if (version) {
      Object.assign(options, { latestVersion, version });
    } else if (!version && isInteractive) {
    ...
    if (dist.repo) {
      ...
      const { github, npm, afterReleaseCommand } = dist;
