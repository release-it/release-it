...
const validateRepoState = async options => {
  ...
  if (!await git.isGitRepo()) {
    throw new Error(options.errors['not-a-git-repo']);
  }

  const remoteUrl = await git.getRemoteUrl();
  if (!remoteUrl) {
    throw new Error(options.errors['no-remote-git-url']);
  }

  if (requireCleanWorkingDir && !await git.isWorkingDirClean()) {
    throw new Error(options.errors['working-dir-must-be-clean']);
  }

  const hasUpstream = await git.hasUpstream();
  if (requireUpstream && !hasUpstream) {
    throw new Error(options.errors['no-upstream']);
  }
