...
const runTasks = async (opts, di) => {
  ...
  try {
    ...
    const { beforeStart, beforeBump, afterBump, beforeStage, afterRelease, afterPrepareRepo } = options.scripts;
    ...
    for (const plugin of plugins) {
      await plugin.prepareRepo();
    }

    await runScript(afterPrepareRepo);
