...
const runTasks = async (opts, injected = {}) => {
  ...
  try {
    ...
    if (dist.repo) {
      prepareDistRepo({ options: dist, spinner, shell, gitClient: gitDistClient });
    }
    ...
    if (dist.repo) {
      ...
      const distClients = await getDistRepoClients({ options, log, isDryRun, remoteUrl, changelogs, shell });
