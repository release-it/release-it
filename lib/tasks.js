...
function parseVersion(options) {
    ...
    if(options.hasOwnProperty("i")) {
        if(options.i == "major" || options.i == "minor" || options.i == "patch") options.i = "pre" + options.i;
    }
    if(options.increment == "major" || options.increment == "minor" || options.increment == "patch") options.increment = "pre" + options.increment;
    var version = util.isValidVersion(options.increment) ? options.increment : options.npm.version;
    log.warn(options.increment);
    // log.warn(option.increment);
    if(!version) {
        ...
        log.warn(util.increment(version, options.increment, options.prereleaseId));
