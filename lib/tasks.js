    ...
    if(!version) {
        ...
        return git.getLatestTag().then(function(tag) {
            if(tag) {
                ...
                log.bold(util.format('From the last tag (%s), the next version is derived: %s', tag, nextVersion));
        ...
        options.prevVersion = options.npm.version;
...
function checkGithubToken(options) {
    if(options.github.release) {
        var token = options.github.token = git.getGithubToken(options.github.tokenRef);
        if(!token) {
