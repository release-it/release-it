...
import { bump, runTemplateCommand as run, pushd, copy, npmPublish, popd, mkTmpDir } from './shell';
...
export default async function runTasks(options) {
  ...
  try {
    ...
    if (!await git.isGitRepo()) {
      if (!options.allowFromSubmodule) {
        throw new Error('Not a git repository.');
      } else {
        if (!await git.isSubmodule()) {
          throw new Error('Not a git submodule.');
        }
      }
    }

    const parsed = await util.parseVersion(options);
    ...
    if (isInteractive && !isVerbose && changelog) {
      log.info(util.truncateLines(changelog));
    }
