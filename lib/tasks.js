...
function getSrcRepoTasks(options) {
    ...
    var isGithubRelease = !(options['non-interactive'] && !options.githubRelease),
        isPublish = !(options['non-interactive'] && !options.publish) && !options.distRepo;

    return _.extend({}, getGenericTasks(options), {
        stage: git.stage.bind(null, options.pkgFiles),
        release: isGithubRelease ? git.release.bind(null, options) : noop,
