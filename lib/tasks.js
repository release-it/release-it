...
function releaseSourceRepo(options) {
  ...
  const repo = getSrcRepoTasks(options);
...
function releaseDistRepo(options) {
  ...
  const repo = getDistRepoTasks(options);
...
function getSrcRepoTasks(options) {
  ...
  const isMakeBaseDir = options.buildCommand && options.dist.repo && options.dist.baseDir, 
...
function getDistRepoTasks(options) {
  ...
  const isPublish = !options['non-interactive'] || (options.npm.publish && !!options.dist.repo), 
