...
class Shell {
  ...
  async execFormattedCommand(command, options = {}) {
    ...
    const useCache = !isExternal && options.cache !== false;
    const isCached = useCache && this.cache.has(cacheKey);
    ...
    delete executionOptions.cache;
    const result =
      typeof command === 'string'
        ? this.execStringCommand(command, executionOptions, { isExternal })
        : this.execWithArguments(command, executionOptions, { isExternal });

    if (useCache && !this.cache.has(cacheKey)) {
      this.cache.set(cacheKey, result);
      Promise.resolve(result).catch(() => this.cache.delete?.(cacheKey));
    }
