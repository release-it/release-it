...
test.serial('should throw if on wrong branch', async t => {
  ...
  childProcess.execSync('git remote remove origin');
...
test.serial('should not throw if one of required branch matches', async t => {
  ...
  const gitClient = factory(Git, { options });
  childProcess.execSync('git checkout -b release/v1');
  await t.notThrowsAsync(gitClient.init());
});
...
test.serial('should detect and include version prefix ("v")', async t => {
  const gitClient = factory(Git, { options: { git } });
  childProcess.execSync('git tag v1.0.0');
...
test.serial('should get the latest tag after fetch', async t => {
  ...
  childProcess.execSync(`git clone ${bare} ${other}`);
...
test.serial('should get the latest custom tag after fetch when tagName is configured', async t => {
  ...
  childProcess.execSync(`git clone ${bare} ${other}`);
  process.chdir(other);
  childProcess.execSync('git tag TAGNAME-OTHER-v2.0.0');
  childProcess.execSync('git tag TAGNAME-v1.0.0');
  childProcess.execSync('git tag TAGNAME-OTHER-v2.0.2');
  childProcess.execSync('git push --tags');
  process.chdir(target);
  await gitClient.init();
...
test.serial('should get the latest tag based on tagMatch', async t => {
  ...
  childProcess.execSync('git tag 1.0.0');
  childProcess.execSync('git tag 21.04.3');
  childProcess.execSync('git tag 1.0.1');
  childProcess.execSync('git push --tags');
  await gitClient.init();
...
test.serial('should get the latest tag based on tagExclude', async t => {
  ...
  childProcess.execSync('git tag 1.0.0');
  childProcess.execSync('git commit --allow-empty -m "commit 1"');
  childProcess.execSync('git tag 1.0.1-rc.0');
  childProcess.execSync('git tag 1.0.1');
  childProcess.execSync('git commit --allow-empty -m "commit 2"');
  childProcess.execSync('git tag 1.1.0-rc.0');
  childProcess.execSync('git push --tags');
  await gitClient.init();
...
test.serial('should get the full changelog since latest major tag', async t => {
  ...
  childProcess.execSync('git tag 1.0.0');
  gitAdd('line', 'file', 'Add file');
  childProcess.execSync('git tag 2.0.0-rc.0');
  gitAdd('line', 'file', 'Add file');
  childProcess.execSync('git tag 2.0.0-rc.1');
  gitAdd('line', 'file', 'Add file');
  await gitClient.init();
  t.is(gitClient.config.getContext('latestTag'), '1.0.0');
  const changelog = await gitClient.getChangelog();
